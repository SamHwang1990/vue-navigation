...
export default function(routeConfigs, stackConfig = {}) {
  ...
  let { initialRouteParams } = stackConfig;
  ...
  routeNames.forEach(routeName => {
    ...
    paths[routeName] = { re, keys, toPath: compile(pathPattern) };
  });
  ...
  return {
    ...
    getStateForAction(action, state) {
        ...
        for (let i = 0; i < childRouterNames.length; i++) {
          ...
          if (childRouter) {
            ...
            if (navigatedChildRoute === null) {
              ...
              routeToPush = childRouter.getStateForAction(
                  action
              );
            }
            if (routeToPush) {
      ...
      if (action.key && action.type === NavigationActions.BACK_TO) {
        const backToRoute = state.routes.find(
          route => route.key === action.key
        );
        if (backToRoute) {
          let backToRouteIndex = state.routes.indexOf(backToRoute);

...
function uniqueScenesWithIndex(scenes) {
  const indexPrevScenes = {};

  scenes.splice(0, scenes.length).forEach(scene => {
    const { index } = scene;
    const indexPrevScene = indexPrevScenes[index];
  ...
  for (let index in indexPrevScenes) {
...
export default function ScenesReducer(scenes, nextState, prevState) {
  ...
  nextState.routes.forEach((route, index) => {
    const key = SCENE_KEY_PREFIX + route.uid;
    const scene = {
      index,
      isActive: false,
      isStale: false,
  ...
  if (
    nextScenes.some((scene, index) => !areScenesShallowEqual(scenes[index], scene))
  ) {
    return nextScenes;
  }
